...
// make plastic stuff public and noDoc - cool
...
class ServiceProxyBuilder {
	...
	Obj buildProxy(Type serviceType) {
		...
		lazyField := pod.type(model.className).field("lazyService")

		plan := Field:Obj?[:]
		plan[lazyField] = LazyService(registry, serviceType.name)

//		plan[]
//		Field.makeSetFunc(vals)
	...
	static Void main(Str[] args) {
		ServiceProxyBuilder().buildProxy(DependencyProviderSource#)
	}
}

internal class Plastic {
