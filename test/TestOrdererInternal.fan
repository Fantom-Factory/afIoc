...
@Js
internal class TestOrdererInternal : IocTest {
	...
	** See http://en.wikipedia.org/wiki/Directed_acyclic_graph
	...
	Void testOneErr() {
		orderer := Orderer()
		addNode(orderer,"1", ["1"])
		verifyIocErrMsg(ErrMsgs.orderer_configRecursion(["1", "1"])) {
			orderer.order
		}
	}
	...
	Void testTwoErr() {
		...
		addNode(orderer,"1", ["2"])
		addNode(orderer,"2", ["1"])
		verifyIocErrMsg(ErrMsgs.orderer_configRecursion(["1", "2", "1"])) {
			orderer.order
		}
	}
...
//		addNode(orderer, "3")
		...
		verifyIocErrMsg(ErrMsgs.orderer_configRecursion("10 3 9 8 3".split)) {
			orderer.order
		}		
	}
