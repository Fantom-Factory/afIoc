...
const class ChefsSpecials {
	const Str[] dishes := ["Pasta Surprise"]
}
...
class Main {
    Void main() {
        ...
        menu1 := (DinnerMenu) scope.serviceById("Example2_0::DinnerMenu")  // returns a service instance
        menu2 := (DinnerMenu) scope.serviceByType(DinnerMenu#)             // returns the same instance
        menu3 := (DinnerMenu) scope.build(DinnerMenu#, [["Soup"]])         // build a new instance
